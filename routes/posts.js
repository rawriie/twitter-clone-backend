...
//Create new post
router.post("/", protect, upload.single("uploadedFile"), async (req, res) => {
  try {
    const img = await uploadToCloudinary(req.file.buffer, false);
...
router.get("/user/:id", protect, async (req, res) => {
  try {
    const replies = await Posts.find({ parentId: req.params.id })
      .populate("userId", "username displayName profilePic")
      .populate("likesCount")
      .populate("replyCount")
      .sort({ createdAt: -1 })
...
router.get("/user/replies/:userId", protect, async (req, res) => {
  try {
    ...
    const replies = await Posts.find({
